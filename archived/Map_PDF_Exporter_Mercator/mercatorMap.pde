...
public class MercatorMap {
  ...
  private float getScreenYRelative(float latitudeInDegrees) {
    //return log(tan(latitudeInDegrees / 360f * PI + PI / 4));
     return  log(tan(PI/4 + getRadians(latitudeInDegrees)/2));
  }
  ...
  protected float getScreenY(float latitudeInDegrees) {
      ...
      float latRad = getRadians(latitudeInDegrees);


    
      // get y value
     float mercN = log(tan((PI/4)+(latRad/2)));
     return mapScreenHeight-((mapScreenWidth*mercN)/(PI/2));//+(mapScreenHeight/4);
     
     // return   (mapScreenHeight/2)-(mapScreenWidth*mercN/(2*PI));
*/  
//}
